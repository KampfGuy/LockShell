...
with sync_playwright() as p:
    ...
    check(page.inner_text('#appTitle') == 'Sky Hop', 'Sky Hop starts'); page.click('#appBack'); page.wait_for_timeout(300)
    page.click('.game-card:has-text("Moon Rocket")'); page.wait_for_timeout(900)
    spr = page.evaluate('fetch("img/saturn-v.png").then(r => r.ok)')
    st = page.evaluate('document.querySelector("canvas.mr-canvas")._game.state')
    box = page.locator('canvas.mr-canvas').bounding_box(); page.mouse.move(box['x'] + box['width'] / 2, box['y'] + box['height'] - 40); page.mouse.down(); page.mouse.move(box['x'] + box['width'] / 2 + 60, box['y'] + box['height'] * 0.7, steps=5); page.wait_for_timeout(800); page.mouse.up()
    check(spr and st == 'ready' and page.evaluate('document.querySelector("canvas.mr-canvas")._game.state') in ('play', 'crash'), 'Moon Rocket: Saturn V sprite loads and a drag launches')
    page.click('#appBack'); page.wait_for_timeout(300)
    page.click('.game-card:has-text("Block World")'); page.wait_for_timeout(900)
    check(page.evaluate('!!document.querySelector("canvas.bw-canvas")._bw') and page.locator('.bw-slot').count() >= 10, 'Block World opens with a hotbar')
    page.click('#appBack'); page.wait_for_timeout(200); page.click('#appBack'); page.wait_for_timeout(300)
    page.evaluate('localStorage.setItem("lockshell.best.snake", "9")'); page.click('#homeLock'); page.wait_for_timeout(400)
    for k in '1234': tap(k)
    page.wait_for_timeout(900)
    check(act('#home') and page.evaluate('localStorage.getItem("lockshell.best.snake")') is None, 'best scores reset after lock + unlock')
