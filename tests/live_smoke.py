...
with sync_playwright() as p:
    ...
    check(page.evaluate('LS.VERSION') == '1.2.0', 'LS.VERSION 1.2.0')
    ...
    check(all(x in labels for x in ['Piano', 'Calendar', 'Dice', 'Shell']) and len(labels) == 15, 'new apps on home by default (15 tiles)')
    ...
    check('Limit Adult Websites' in page.inner_text('#appBody') and '19845' not in page.content(), 'About expanded, no dev code'); page.click('#appBack'); page.wait_for_timeout(300)
    ...
    addr('youtube.com'); check(page.locator('.sh-blocked').count() == 1, 'non-allowlisted site blocked')
    ...
    check('lockshell-v1.2.0' in caches, 'SW cache lockshell-v1.2.0 (' + ','.join(caches) + ')')
